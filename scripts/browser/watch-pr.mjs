...
async function main() {
  ...
  if (!prUrl || !prUrl.includes('github.com')) {
    ...
    console.error('Example: watch-pr https://github.com/wayfair-shared/sf-ui-web/pull/15218');
    process.exit(1);
  }
  ...
  try {
    ...
    while (true) {
      ...
      if (await needsAuth(page)) {
        ...
        console.error('❌ GitHub authentication required. Please login manually.');
        process.exit(1);
      }

      // Check if PR is already merged
      const isMerged = await page.locator('.State--merged').isVisible({ timeout: 1000 }).catch(() => false);
      if (isMerged) {
      ...
      const isClosed = await page.locator('.State--closed').isVisible({ timeout: 1000 }).catch(() => false);
      if (isClosed) {
      ...
      const resolveButton = page.locator('button:has-text("Resolve conflicts")');
      const hasConflicts = await resolveButton.isVisible({ timeout: 1000 }).catch(() => false);

      if (hasConflicts) {
        console.log('🔧 Auto-resolving conflicts...');
        await notify('🔧 Resolving Conflicts', `PR #${prNumber}`, `${owner}/${repo}`);
        ...
        try {
          ...
          if (await updateButton.isVisible({ timeout: 1000 }).catch(() => false)) {
            ...
            console.log('⬆️  Updated branch');
            await notify('⬆️  Branch Updated', `PR #${prNumber}`, `${owner}/${repo}`);
          }
        } catch (err) {
          console.error('❌ Failed to resolve conflicts:', err.message);
          await notify('❌ Conflict Resolution Failed', `PR #${prNumber}`, err.message);
          process.exit(1);
        }
      ...
      const canMerge = await squashButton.isEnabled({ timeout: 1000 }).catch(() => false);

      if (canMerge) {
        console.log('✅ Squash and merge is available!');
        await notify('✅ Ready to Merge!', `PR #${prNumber}`, `${owner}/${repo} - Squash and merge available`);
        console.log('Stopping - merge button is ready. Visit PR to complete merge.');
        break;
      }
