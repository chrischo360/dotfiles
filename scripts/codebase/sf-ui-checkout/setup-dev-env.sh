...
# 
# This script automates the complete setup of the sf-ui-checkout development environment
...
# Change to the sf-ui-checkout directory for all operations
cd "$HOME/codebase/sf-ui-checkout"

# Configuration
readonly SF_CHECKOUT_DIR="$HOME/codebase/sf-ui-checkout"
...
show_header() {
    log_celebration "🚀 SF-UI-Checkout Development Environment Setup"
    log_info "Starting automated development environment configuration..."
    log_info "Working directory: $SF_CHECKOUT_DIR"
...
validate_prerequisites() {
    ...
    if [[ ! -f "package.json" ]] || [[ ! -d "src" ]]; then
        log_error "Not in sf-ui-checkout directory. Please run from: $SF_CHECKOUT_DIR"
        return 1
    fi
...
show_final_status() {
    ...
    echo "  📦 Webpack Dev Server: http://localhost:8898/webpack/sf-ui-checkout"
    echo "  🛒 Cart Page: https://wayfaircom.csnzoo.com/v/checkout/basket/show"
    echo "  💳 Checkout Page: https://secure.wayfaircom.csnzoo.com/v/checkout/onepage/view?ft_override_enable_webpack_checkout=ON&webpack-localhost-apps[]=sf-ui-checkout&devbox=sde-php8ccho"
    echo ""
    ...
    # Open checkout page in browser if setup was successful
    if [[ "$CRITICAL_FAILURE" == "false" ]]; then
        local checkout_url="https://secure.wayfaircom.csnzoo.com/v/checkout/onepage/view?ft_override_enable_webpack_checkout=ON&webpack-localhost-apps[]=sf-ui-checkout&devbox=sde-php8ccho#shipping"
        
        if [[ -n "$BROWSER_CHOICE" ]]; then
            log_web "Opening checkout page in $BROWSER_CHOICE..."
        else
            log_web "Opening checkout page in default browser..."
        fi
        
        if open_in_browser "$checkout_url" "$BROWSER_CHOICE"; then
            if [[ -n "$BROWSER_CHOICE" ]]; then
                log_success "Checkout page opened in $BROWSER_CHOICE"
            else
                log_success "Checkout page opened in default browser"
            fi
        else
...
OPTIONS:
    ...
    -b, --browser NAME  Specify browser to open checkout page (chrome, chromium, firefox, safari, edge, brave)
...
DESCRIPTION:
    Automates the complete setup of the sf-ui-checkout development environment
    including tmux session management, process monitoring, remote sync, SSH
    connections, and automated cart verification.

EXAMPLES:
    $0                          # Normal setup with default browser
    $0 --debug                  # Setup with debug output for troubleshooting
    $0 --browser chrome         # Setup and open checkout page in Chrome
    $0 --browser chromium       # Setup and open checkout page in Chromium
    ...
    $0 --install-chromium -b chromium --debug  # Install Chromium, use it, with debug mode
