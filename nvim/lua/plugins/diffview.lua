return {
	...
	keys = {
		...
		-- File history for entire repository
		{
			'<leader>dfa',
			'<cmd>DiffviewFileHistory<cr>',
		...
		-- Compare between two branches
		{
			'<leader>dbb',
			function()
				local branch1 = vim.fn.input('First branch: ')
				if branch1 ~= '' then
					local branch2 = vim.fn.input('Second branch: ')
					if branch2 ~= '' then
		...
		-- Compare with HEAD~n
		{
			'<leader>dch',
			function()
				local commits = vim.fn.input('Compare with HEAD~n (n=): ')
				if commits ~= '' then
					vim.cmd('DiffviewOpen HEAD~' .. commits)
				end
			end,
			desc = 'Compare with HEAD~n',
		},
