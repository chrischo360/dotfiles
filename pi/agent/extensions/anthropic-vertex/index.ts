 ...
 *
 * Routes Claude models through Google Vertex AI using Application Default Credentials.
 *
 * Prerequisites:
 *   gcloud auth application-default login
 *   export GOOGLE_CLOUD_PROJECT=your-project-id
 *   export GOOGLE_CLOUD_LOCATION=us-east5  # or us-central1, europe-west1, etc.
 *
 * Usage:
	...
	(async () => {
		...
		try {
			const client = new AnthropicVertex({
				...
				region: process.env.GOOGLE_CLOUD_LOCATION || "us-east5",
			});
			...
			const anthropicStream = (client as any).messages.stream(
				{ ...params },
				{
...
export default function (pi: ExtensionAPI) {
	pi.registerProvider("anthropic-vertex", {
		baseUrl: `https://${process.env.GOOGLE_CLOUD_LOCATION || "us-east5"}-aiplatform.googleapis.com`,
		apiKey: "GOOGLE_CLOUD_PROJECT",
		api: "anthropic-vertex-api",
		...
		streamSimple: streamAnthropicVertex,
	});
}
