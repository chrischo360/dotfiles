...
# Automatically switches between light (Dawn) and dark (Main) themes based on time
...
TMUX_THEME_SCRIPT="$THEME_DIR/tmux-theme-rosepine.sh"
...
switch_alacritty() {
    local theme=$1
    local colors_file="$THEME_DIR/alacritty-colors-rosepine-${theme}.toml"
    ...
    # Look for either "Kanagawa" or "Rose Pine" comment
    sed -n '1,/^# \(Kanagawa\|Rose Pine\)/p' "$ALACRITTY_CONFIG" | head -n -1 > "$temp_file"
    ...
    echo -e "${GREEN}✓ Alacritty theme switched to Rose Pine $theme${NC}"
}
...
switch_tmux() {
    ...
    if [ -n "$TMUX" ]; then
        ...
        echo -e "${GREEN}✓ Tmux theme switched to Rose Pine $theme${NC}"
    else
...
switch_theme() {
    ...
    echo -e "${BLUE}Switching to Rose Pine $target_theme theme...${NC}"
    ...
    echo -e "${GREEN}✓ Theme switched to Rose Pine $target_theme${NC}"
...
case "${1:-auto}" in
    ...
    *)
        ...
        echo "  dark   - Switch to dark theme (Rose Pine Main)"
